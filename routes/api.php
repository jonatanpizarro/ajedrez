...
Route::get('/mou/{jugador}/{id_partida}/{pos_ini}/{pos_dest}' , function($jugador,$id_partida , $pos_ini ,$pos_dest){
	...
	if ($posicion1[0]['posicionIni']==$pos_ini) {
		$pos=Fichas::where('jugador',$jugador)->where('id_partida',$id_partida)->update(['posicionFin'=>$pos_dest]);

		Fichas::where('jugador',$jugador)->where('id_partida',$id_partida)->update(['posicionIni'=>$pos]);
	...
	header("Access-Control-Allow-Origin: *");	
	return json_encode(array('estado'=>'ok','posIni'=>$posicion2[0]['posicionIni'],'posFin'=>$posicion[0]['posicionFin'] ));
...
Route::get('/ver/{jugador}/{id_partida}' , function($jugador,$id_partida){
	...
	header("Access-Control-Allow-Origin: *");	
	return json_encode(array('estado'=>'ok','pos'=>$posicion1[0] ));
